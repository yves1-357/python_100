...
time.sleep(5)
timeout = time.time() + 5
five_min_stop = time.time() + 60*5
search = driver.find_element(By.ID, value="bigCookie")

# while True:
#     search.click()
#     if time.time() > five_min_stop:
#         break
#     if time.time() > timeout:
#         timeout = time.time() + 5
#         try:
#             boutique = driver.find_element(By.XPATH, '//*[@id="product0"]')
#             produit_boutique = driver.find_element(By.ID, value="productName2")
#             produit_boutique.click()
#             timeout = time.time() + 5
...
time.sleep(5)
timeout = time.time() + 5
five_min_stop = time.time() + 60*1
search = driver.find_element(By.ID, value="bigCookie")

while True:
    search.click()
    if time.time() > five_min_stop:
        print("le total de clicl est fini")
        break
print("Achat des améliorations...")
boutique = driver.find_element(By.XPATH, '//*[@id="product0"]')
# produit_boutique = driver.find_element(By.ID, value="productName2")
boutique.click()
